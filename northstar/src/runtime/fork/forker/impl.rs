...
impl InitProcess {
    fn new(
        ...
        // Spawns async task that owns stream and uses it to communicate with the init process.
        // InitProcess communicates with this task throug the two channels.
        //
        //  ┌─────────────┐   exec   ┌────────────┐
        //  │             │─────────►│            │    socket    ┌──────┐
        ...
        tokio::spawn(async move {
            // This map is used to track the running processes in the contaner and send exit status
            // of the processes as they exit.
                    ...
                    msg = stream.recv() => {
                        match msg {
                            ...
                            Ok(Some(init::Message::Exit { pid, exit_status })) => {
                                let tx = ps.remove(&pid).expect("PID not registered");
